...
export class UpdateProductComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.newsId) {
      this.productService.getProductById(+this.newsId).subscribe(data => {
  ...
  onUpdate() {
    ...
    let oldPathsRaw = this.oldProduct?.contentStoragePathsJson || '[]';
    let parsed: any;
    ...
    const imageChanged = this.oldProduct?.imageStoragePath !== updateProduct.imageStoragePath;

    const deletePromises: Promise<void>[] = [];
    if (imageChanged && this.oldProduct?.imageStoragePath) {
      deletePromises.push(this.firebaseStorageService.deleteFileByPath(this.oldProduct.imageStoragePath));
    }
    ...
    Promise.all(deletePromises).then(() => {
      this.productService.updateProduct(updateProduct.id!, updateProduct).subscribe({
        next: (data) => {
  ...
  onCancel() {
    if (!this.isUpdated) {
      ...
      if (newAvatarPath && newAvatarPath !== this.oldProduct?.imageStoragePath) {
